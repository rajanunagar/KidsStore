...
<title>Cart Page - KidsStore</title>
...
<body>
					...
					<div class="single-sidebar">
						...
						if(r==null || r.size()==0){
                         %>
                            ...
                            <h2><a href="single-product?id=c1">T-shirt</a></h2>
                            <div class="product-sidebar-price">
                                <ins>Rs.174.6</ins> <del>Rs. 180</del>
                            </div>                             
                        </div>
                        <div class="thubmnail-recent">
                            <img src="./static/img/c2.jpg" class="recent-thumb" alt="">
                            <h2><a href="single-product?id=c2">Pent</a></h2>
                            <div class="product-sidebar-price">
                                <ins>Rs.135.0</ins> <del>Rs.150</del>
                            </div>                             
                        </div>
                        <div class="thubmnail-recent">
                            <img src="./static/img/t1.jpg" class="recent-thumb" alt="">
                            <h2><a href="single-product?id=t1">Toy</a></h2>
                            <div class="product-sidebar-price">
                                <ins>Rs. 138.0</ins> <del>Rs. 150</del>
                            </div>                             
                        </div>
                        <div class="thubmnail-recent">
                            <img src="./static/img/t7.jpg" class="recent-thumb" alt="">
                            <h2><a href="single-product?id=t7">Video game</a></h2>
                            <div class="product-sidebar-price">
                                <ins>Rs. 190.0</ins> <del>Rs. 200</del>
                            </div>                             
                        </div>
                         <% 							
						}
						...
						for (int i = 0; i < r.size() && i <= 3; i++) {
							...
							int price = Integer.valueOf(r.get(i).getProduct().getPrice());
							double dis = Double.valueOf(r.get(i).getProduct().getDiscount());
							dis=100-dis;
							out.print("<div class='thubmnail-recent'><img src='./static/img/" + id
							+ ".jpg' class='recent-thumb' alt=''><h2><a href='single-product?id="+id+"'>" + name
							...
							<table cellspacing="0" class="shop_table cart">
								...
								<tbody>
									...
									if (cart == null ) {
										...
										List<CartList> cartlist = cart.getProduct();
										if (cartlist.isEmpty()) {
											out.print("<h1>Your cart is empty!!</h1>");
										} else {
											for (int i = 0; i < cartlist.size(); i++) {
										int quantity = cartlist.get(i).getQuantity();
										total += quantity * (cartlist.get(i).getProduct().getPrice());
										System.out.println(cartlist.get(i).getProduct().getPid());
										out.print(
												"<tr class='cart_item'><td class='product-remove'><button class='button'><a title='Remove this item' class='remove' href='removecart?pid="
														+ cartlist.get(i).getProduct().getPid()
														+ "'>Remove</a></button>  </td><td class='product-thumbnail'><a href='single-product?id="
														+ cartlist.get(i).getProduct().getPid()
														+ "'><img width='145' height='145' alt='poster_1_up' class='shop_thumbnail' src='./static/img/"
														+ cartlist.get(i).getProduct().getPid()
														+ ".jpg'></a></td><td class='product-name'><a href='single-product?id="
														+ cartlist.get(i).getProduct().getPid() + "'>" + cartlist.get(i).getProduct().getName()
														+ "</a> </td><td class='product-price'><span class='amount'>Rs. "
														+ cartlist.get(i).getProduct().getPrice()
														+ "</span> </td><td class='product-quantity'><div class='quantity buttons_added'><form action='updatecart'><input type='hidden' name='pid' value='"
														+ cartlist.get(i).getProduct().getPid()
														+ "'><input type='number' size='4' class='input-text qty text' name='q' title='Qty' value='"
														+ quantity
														+ "' min='0' step='1'>&nbsp&nbsp&nbsp<input type='submit' value='Update' class='button'></form></div></td><td class='product-subtotal'><span class='amount'>Rs. "
														+ quantity * (Integer.valueOf(cartlist.get(i).getProduct().getPrice()))
														+ "</span> </td></tr>");
										/*                                         	   out.print("<tr class='cart_item'><td class='product-remove'><a title='Remove this item' class='remove' href='removecart?pid="+cartlist.get(i).getProduct().getPid()+"'>Remove</a> <a title='Remove this item' class='remove' href='updatecart?pid="+cartlist.get(i).getProduct().getPid()+",q=2'>Update</a></td><td class='product-thumbnail'><a href='single-product'><img width='145' height='145' alt='poster_1_up' class='shop_thumbnail' src='./static/img/"+cartlist.get(i).getProduct().getPid()+".jpg'></a></td><td class='product-name'><a href='single-product'>"+cartlist.get(i).getProduct().getName()+"</a> </td><td class='product-price'><span class='amount'>Rs. "+cartlist.get(i).getProduct().getPrice()+"</span> </td><td class='product-quantity'><div class='quantity buttons_added'><input type='number' size='4' class='input-text qty text' name='q' title='Qty' value='"+quantity+"' min='0' step='1'></div></td><td class='product-subtotal'><span class='amount'>Rs. "+quantity*(Integer.valueOf(cartlist.get(i).getProduct().getPrice()))+"</span> </td></tr>");
										 */ }
										}
									}
									%>
									<!-- <tr class="cart_item">
                                            ...
                                            <td class="product-thumbnail">
                                                <a href="single-product"><img width="145" height="145" alt="poster_1_up" class="shop_thumbnail" src="./static/img/c2.jpg"></a>
                                            </td>
 ...
 -->                                   <form action="checkout" method="post">
                                      <input type="submit" value="Checkout" name="proceed"
											class="checkout-button button alt wc-forward"></form>
										</td>
									</tr>
									<tr>
										<th colspan="6" class="product-subtotal">Total : <b>
												<%
												out.print(total);
												%>
										</b></th>
								...
								<div class="cross-sells">
									...
									<ul class="products">
										...
										for (int i = 0; i < p.size() && i <= 1; i++) {
											out.print(
											"<li class='product'><a href='single-product?id="+p.get(i).getPid()+"'><img width='325' height='325' alt='T_4_front' class='attachment-shop_catalog wp-post-image' src='./static/img/"
													+ p.get(i).getPid() + ".jpg'><h3>" + p.get(i).getName()
													+ "</h3><span class='price'><span class='amount'>Rs. " + p.get(i).getPrice()
													+ "</span></span></a><a class='add_to_cart_button' data-quantity='1' data-product_sku='' data-product_id='22' rel='nofollow' href='single-product?id="+p.get(i).getPid()+"'>Select options</a></li>");
										}
										}
										else{
										%>
                                        ...
                                        <a href="single-product">
                                            <img width="325" height="325" alt="T_4_front" class="attachment-shop_catalog wp-post-image" src="./static/img/t1.jpg">
                                            <h3>Ship Your Idea</h3>
                                            <span class="price"><span class="amount">Rs. 150</span></span>
                                        </a>
                                    ...
                                    <li class="product">
                                        <a href="single-product">
                                            <img width="325" height="325" alt="T_4_front" class="attachment-shop_catalog wp-post-image" src="./static/img/c2.jpg">
                                            <h3>Ship Your Idea</h3>
                                            <span class="price"><span class="amount">Rs. 150</span></span>
                                        </a>
